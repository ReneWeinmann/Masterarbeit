...
\begin{algorithm}[H]
	...
	\WHILE{Anzahl der Durchlaeufe < maximale Anzahl an Durchlaeufen}
...
%		
		...
		\FOR{Anzahl an Fortpflanzungen>0} \STATE{
			\textbf {Auswahl:} Auswahl der Elternpaare \(C_{1}\)+\(C_{2}\) }				\STATE{ mit Roulette-Wheel-Selection aus \(P_{N}\)}
	 	...
	 	\ENDFOR
	  	\FOR{Anzahl an neuen Chromosomen}
	 		\STATE{\textbf {Create:} Erstellen von neuem Chromosom \(C_{4}\)}
	 		\STATE{\textbf {Einfuegen:} Einfuegen von \(C_{4}\) in die Population\(P_{N}\)}
		\ENDFOR
