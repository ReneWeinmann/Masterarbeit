...
\section{Ansatz}
Die Register-Allokation ist dafür zuständig,dass alle Register sowohl physikalisch als auch virtuelle im Register-File abgespeichert werden können. Dazu wird diese nach jedem Scheduling einer SLM aufgerufen und geht hierbei alle Instruktionen und die darin verwendeten Register durch . Bei Verwendung eines physikalischen Registers überprüft die Allokation, ob die vom Programmierer gewählte Register-Adresse und Register-File bereits blockiert oder nicht möglich. Handelt es sich um ein virtuelles Register wird zuerst eine geeignete Adresse zugewiesen. Hierbei wird ebenfalls darauf geachtet das diese zugewiesen wird welche nicht blockiert oder nicht möglich ist.
...
Wie bereits im Grundlagenteil \ref{chap:grundlagen_cossover} skizziert handelt es sich beim Crossover um das Fortpflanzen der Chromosomen. Dieses Verfahren wird auf die komplette alte Population ausgeführt und dabei so oft wiederholt, bis eine neue Population mit Chromosomen befüllt wurde.
Das Fortpflanzungspaar wird hierbei mittels dem bereits erwähnten Roulette Wheel Selection  \ref{chap:grundlagen_cossover} ausgewählt, dabei handelt es sich um zwei individuelle Chromosomen aus der alten Population. Dieses Paar wird daraufhin an einer zufällig gewählten Stelle getrennt und die entstandenen Teile zusammengeführt.Hierbei hat die Literatur gezeigt, dass ein Crossover mit mehr als einem Crossover Punkt ein besseres Ergebnis liefert. 
...
Da die Registerallokation dem Scheduling untergeordnet ist und von diesem aufgerufen wird, muss die Fitnessfunktion an die des Scheduling angepasst werden. Aus diesem Grund wurde zum einfacheren Vorgehen zwei Verschiedene Fitness-Funktionen implementiert. Zum einen der Wert welcher an die übergeordnete Funktion (das Scheduling) weitergegeben wird und zum Andern eine interne Fitness um die Ergebnisse zu evaluieren und geeignete Fortpflanzungspaare zu finden(Rolette-Wheele-Selection).
Der Übergabewert besteht aus der Anzahl an Registern die nicht im Register-File allokiert werden konnten. Das sortieren der Population wird mit beiden Fitness-Funktionen realisiert. Da die Register-Allokation nur dann erfolgreich war wenn alle Register allokiert werden konnten, ist dies das erste Attribut nach dem die Population sortiert wird. Als zweiter Sortierschlüssel wird die interne Fitness-Funktion verwendet. Bei dieser wurden mehrere Ansätze evaluiert.
