...
Dabei ist zu erkennen, das die Heuristik eine deutlich bessere Loesung findet. Die Verschlechterung der Schaltleistung in der neuen Heueristik, laesst sich mit unterschiedlichen Lastkapazitaeten begruenden. Durch eine Beruecksichtigung dieser wuede die Heuristik die Verlustleistung noch besser modellieren. 
...
Die prozentuale Veränderung der Internal-Leistung liegt im sehr niedrigen Prozentbereich und wir ebenfalls nicht weiter beruecksichtigt. 
...
Da der verwendete Prozessor keine Gleitkommavariablen unterstützt, besteht ein Algorithmus der diese emuliert. Dabei wird eine Gleitkommazahl als zwei Subworte deklariert, zum einen der Siknifiant $T$ und zum anderen der Exponent $E$. Mit hilfe dieser kann eine Gleitkommazahl $f$ nach Formel \ref{eq:emulated_floating_point} berechnet werden.
\begin{equation}
f = T *2^E
\label{eq:emulated_floating_point}
\end{equation}
Beide Variablen $ T,E$ koennen als Subwort im Register gespeichert werden. Dabei koennen diese an eine beliebige Adresse des Registers stehen. Aus diesem Grund werden hierfuer zwei Virtuelle Register verwendet. Diese Emulation ist besonders geeignet fuer SIMD-Architekturen, bei der eine Gleitkommaberechnung ohne Aufschluesseln der Variablen errechnet werden kann. Dadruch ist diese Art von Gleitkommaberechnung ausserst perfomant.
Wenn nun ein Algorithmus mit Gleitkommazahlen rechnet, werden demzufolge nur virtuelle Register benoetigt, welches den Algorithmus paraedistiniert fuer eine Adressoptimierung macht und aus diesem Grund Algorithmen verwendet werden die mit diesen arbeiten. \cite{gerlach2016efficient}
