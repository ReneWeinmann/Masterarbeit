...
In diesem Kapitel wird kurz auf die Grundlagen und Begriffe von Signalprozessoren eingegangen. Im Anschluss wird die Verwendete Architektur erläutert und einzelne Bestandteile genauer untersucht. Anschließend wird die Funktion des Scheduling erklärt und das Prinzip der Verlustleistung aufgezeigt. Das Ende des Kapitels soll eine grobe Übersicht über Genetische-Algorithmen gegeben.

\section{FPGA}
Bei einem FPGA (Field Programmable Gate Array) handelt es sich um ein frei programmierbaren integrierten Schaltkreis, welcher mithilfe von Look-Up-Tabellen sehr effizient Algorithmen abbildet. Um eine gewünschte Funktion auf dem FPGA zu implementieren ist es notwendig die Look-Up-Tabellen mit einer Wahrheitstabellen zu befüllen, welche diese Funktion abbildet. Dadurch kann in einem FPGA jeglich erdenkliche Aufgabe erfuellen. Die beschriebnen Blocke muessen ausserdem mithilfe von Rooting-Switches und Connection-Boxen verbunden werden, um eine Verbindung zu den IO-Ports sicherzustellen. Damit der Baustein konfiguriert werden kann, muss die Funktion mithilfe einer HDL(Hardware Description Language) beschrieben werden.\cite{farooq2012fpga} Vorteil dieser Implementierung ist, dass eine echte Parallelität erreicht werden kann, so dass mehrere Blocke zeitgleich ausführbar sind. In dieser Arbeit wird ein FPGA als Entwicklungsumgebung verwendet, um den zu entwickelnden ASIP bestmöglich abzubilden und dabei möglichst flexibel zu bleiben.


\section{ASIP}
ASIP bedeutet so viel wie Application Specific Instruction Set Processor, dabei handelt es sich wie der Name schon sagt um einen anwendungsspezifischen Prozessor. Hierbei ist der ASIP auf eine gewisse Funktion optimiert und fuehrt diese optimal aus. Durch den Einsatz auf ein spezielles Gebiet, koennen Einsparungen hinsichtlich Siliciumflaeche und Energieverbrauch gegenueber herkoemmlichen Prozessoren verzeichnet werden. Der in dieser Arbeit verwendete ASIP ist eine vereinfachte Variante eines MIPS-Prozessors. MIPS oder auch \glqq Microprocessor without interlocked pipeline stages\grqq{} ist eine Befehlssatzarchitektur bei der der Entwickler/Compiler sich um Datenabhaengigkeiten der Befehle kuemmern muss und gegebenen falls Leerbefehle (NOPs) in den Code einfuegen muss.
...
Wie das Wort Pipelining schon besagt, handelt es sich hierbei um eine Befehlsabarbeitung am Fließband(Pipeline). Wurde ein Befehl in der ersten Piplinestufe abgearbeitet kann dieser an die nächste Stufe weitergeleitet werden und der nachfolgende Befehl führt die erste Phase aus. Nachdem die Anzahl so viel Befehle wie Pipline Stufen dem System uebergeben wurden, liegen ab diesem Zeitpunkt alle Daten Parallel am Ausgang an.(siehe Abbildung \ref{fig:pipline})  Somit ist eine parallele Verarbeitung mehrerer unterschiedler Befehle möglich. Dabei ist darauf zu achten, dass die Taktung der Piplinestufen sich an dem Befehel orientiert welcher die laengste Ausfuehrungsdauert hat.
\begin{scriptsize}
	\begin{figure}[htbp] 
		\centering
